    ...
    <SafeAreaView style={screenStyles.flex1} edges={safeAreaEdges}>
      ...
      >
        ...
        {repeatRules.map((rule: RepeatRule, index: number) => {
          ...
          const ruleKey = `rule-${rule.message}-${rule.startNumber ?? 'none'}-${rule.endNumber}-${rule.repeatCount ?? 0}-${rule.endMode ?? 'none'}-${rule.ruleNumber}-${rule.color ?? 'default'}`;
          return (
            <RuleCard
              key={ruleKey}
        ...
        {!isAddingNewRule && (
          <View className="w-full items-center mb-6">
            <View className="relative">
              <TouchableOpacity
                className={`${colorStyles.lightest.containerClassName} rounded-xl pl-5 pr-4 py-2 flex-row items-center`}
                onPress={handleAddRule}
              >
                <Text className={`text-base ${appTheme.tw.text.black} mr-2`}>규칙 추가</Text>
                <Plus size={22} color={appTheme.colors.black} strokeWidth={2.5} />
              </TouchableOpacity>
